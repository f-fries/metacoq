...
Proof.
  intros u v.
  destruct u, v; try (right; congruence); try solve [left; reflexivity].
  assert (H : {UnivExprSet.elements t = UnivExprSet.elements t0}
              + {~ UnivExprSet.elements t = UnivExprSet.elements t0}). {
    repeat decide equality. }
  destruct H as [H|H]; [left; f_equal; now apply eq_universe_iff' in H|right].
  intros [= X]; apply H; now apply eq_universe_iff' in X.
Qed.
